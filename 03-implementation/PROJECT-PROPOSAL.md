...
**Authoritative companions (kept, not superseded):** `METHODOLOGY-AND-IMPLEMENTATION-DESIGN.md` (the build plan + edge-case register), `DE-RISK-INDEX.md` (measured evidence), `THREAT-MODEL-ANALYSIS.md` (code-grounded threat detail), the `*-SPIKE.md` files (Besu/QBFT/MQTT measurements), and `06-literary-survey/` (the 251-paper corpus).
**Last refined:** 2026-06-20, after a four-reviewer adversarial pass (novelty, methodology, threat-model, scope) plus a verified related-work survey.
...
| Identity | DID/VC; vs Ed25519 + permissioned registry | **Ed25519 + permissioned registry + revoke** | DID/VC is 3-5x the work and brittle for a single domain (`proofmember23`). The registry suffices; DID/VC is future work. |
...
- The guarded fast-default / slow-LLM-exception architecture is **VLMLight** (arXiv:2505.19486). We extend it explicitly, citing it as prior art, to the multi-junction and adversarial-channel setting it does not address (it is single-intersection, no trust).
- Multi-agent LLM neighbour coordination is **CoLLMLight** (arXiv:2503.11739). It trusts all neighbour reports and fine-tunes an 8B model; we authenticate and plausibility-check the channel with a frozen 3.8B model.
- LLM-as-emergency-exception-handler is **LA-Light** (arXiv:2403.08337), cloud GPT-4, single junction, benign faults only; our faults are *adversarial and signed*.
- The closest *attack-detection on coordination* is **Keijzer2021** (arXiv:2104.03801), a control-theoretic residual filter for collaborative intersection control. Our delta is not the detection mechanism (we use a lighter conservation check) but the **measured detectability envelope across an attacker-capability axis** (the recall-collapse boundary, Section 7), plus cryptographic identity and a tamper-evident audit it does not have.
...
|---|---|---|---|---|---|---|---|
| **CoLLMLight** (2503.11739, 2025) | Cooperative LLM agents pass spatiotemporal state to neighbours, network-wide | Llama-3.1-8B, **fine-tuned** (+GPT-4o teacher, PPO) | CityFlow; Jinan/Hangzhou/NY, normal flow | **Yes** (closest coordination analogue) | **No** (trusts neighbours) | Trusts neighbour messages; needs fine-tuning; 8B > edge budget | Frozen 3.8B; Ed25519-signed + registry-verified + conservation-checked coordination |
| **VLMLight** (2505.19486, 2025) | Fast classical path + slow LLM reasoning for safety-critical cases; deterministic AgentCheck override | Qwen2.5-VL-32B + 72B; frozen, server-class | Image-SUMO; emergency scenarios | Internal multi-agent dialogue, **not** inter-junction | **No** | 72B not edge; single-junction; no compromised-input model | Same dual-branch idea, 3.8B edge SLM + MaxPressure shield + signed inter-junction coordination |
| **LA-Light** (2403.08337, 2024) | Tool-using LLM calls RL/rule controllers for long-tail (sensor fail, emergency) | GPT-4 + tools, cloud, no FT | SUMO/TSHub; sensor-outage + emergency | No (single intersection) | **No** | Cloud GPT-4, single junction, benign faults, no trust | Local frozen SLM exception handler; adversarial (signed-but-lying) faults |
...
| **REG-TSC** (2510.26242, 2025) | RAG-enhanced distributed LLM agents + emergency handling | GPT-4o-mini + Llama-3.1-8B | CityFlow; Jinan/Hangzhou/Yizhuang | **Yes** (distributed) | **No** | 8B/cloud; RAG dependency; no compromised-input robustness | Frozen 3.8B; RAG optional; signed + conservation-checked |
...
| **MaxPressure** (Varaiya 2013; no arXiv) | Provably throughput-maximising pressure control | Analytical rule, no training | Reimplemented in SUMO/CityFlow | Local pressure (implicitly cooperative) | **No** | Not a competitor: it is our default + shield | Adopted directly as default + deterministic shield |
| **SafeLight** (2211.10871, 2023) | Safety-enhanced residual RL, reward shaping for collision-free TSC | Residual RL, **trained** | RESCO/SUMO; collision scenarios | No | Safety, **no** adversarial model | Needs training; no spoofed-input defence | Deterministic shield (no training) for action safety; trust handled separately |
| **Blockchain-TSC** (1906.02628, 2019) | Blockchain validates connected-vehicle data vs spoofing | No model | I-SIG/CV pilot | No signal coordination | **Yes** (vehicle-data) | Heavyweight consensus; no SLM; secures vehicle->signal, not signal<->signal | Lightweight hash-chained audit + Ed25519 vs permissioned registry |
...
| **Keijzer2021** (2104.03801, 2021) | Model-based residual detection of attacks in collaborative intersection control | Observer/residual, no LLM | Collaborative intersection sim | **Yes** | **Yes** (attack detection) | Control-theoretic filter; no SLM/identity/audit | Conservation check (lighter) + signatures + registry + audit |
| **Xiao2026** (2602.10162, 2026; registry-derived) | Limits of residual-based detection vs physically-consistent FDI | Detection theory | State-estimation/FDI | n/a | **Yes** (evasion analysis) | A caveat, not a system | Treated as a threat-model bound: a key-holding, physics-consistent adversary is out of scope |

**Positioning summary.** The unoccupied gap is the conjunction: a frozen no-training edge SLM as a guarded exception handler over a MaxPressure default+shield, where inter-junction coordination is cryptographically authenticated and screened by a vehicle-conservation plausibility check, evaluated in SUMO under a compromised channel. The three closest threats are CoLLMLight (same coordination, but trusts neighbours, fine-tunes), VLMLight (same dual-branch safety architecture, but 72B and no trust), LA-Light (same exception-handler role, but cloud, single-junction, benign faults). The honest bound (Xiao2026): our layer stops unauthenticated spoofing and gross faults, not a key-holding adversary crafting physically-consistent injections.
...
4. **Colluding insiders (>=2 keys), conservation-respecting:** explicitly **out of scope**; recall = 0 by design (Xiao2026). Contained by `revoke` + audit, not detection.
...
**SHOULD:** predictive-coordination floor + the causal-coordination demonstration (Channel decision resolved); real-SLM n=30 ceiling run with decision-attribution; detection operating-envelope sensitivity analysis; the fused spoof-a-fake-emergency experiment.
