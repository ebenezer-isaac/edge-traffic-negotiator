...
Traffic lights at neighbouring junctions increasingly coordinate (green waves, ambulance preemption). This project puts a small AI (Phi-4-mini, running locally, never retrained) at each junction so they **coordinate to handle emergencies and incidents together**: clearing an ambulance across the corridor, reallocating around a blockage, instead of each junction reacting alone. A classical controller (MaxPressure) stays in charge by default and acts as a safety net; the SLM is called in only on the hard, ambiguous cases.

The moment junctions act on each other's reports, a faulty or hijacked junction can lie, and a naive coordinator acts on the lie: wasted green, starved side streets, or a signal tricked into clearing the road for an ambulance that does not exist. So the coordination runs over a channel that is **digitally signed and sanity-checked**, and the system is built to **degrade safely when a report is compromised**. The real question is *not* "can the AI beat the classical controller on normal traffic" (it cannot, and we say so plainly) but: **does on-device coordination clear emergencies better than isolated control, and stay trustworthy when a junction is compromised?** The headline experiment shows a real ambulance cleared by coordination, and a spoofed fake-emergency from a hijacked junction refused because preemption is gated on *physical corroboration*, not the signature alone.
...
> A **coordinated multi-agent edge-SLM signal system that handles cross-junction emergencies and incidents**, in which a deterministic MaxPressure controller is the default and safety shield, neighbouring junctions coordinate over an **authenticated, plausibility-checked** channel, a frozen edge SLM is a guarded exception handler, and the system **stays robust when the coordination messages are compromised.**

The headline contribution is the **fusion**: on-device SLM coordination across junctions for emergency and incident response, on a frozen 3.8B edge model, that remains trustworthy under an authenticated-but-compromised neighbour. Each axis alone is precedented (coordination: CoLLMLight; emergency exception-handling: VLMLight/LA-Light/EMVLight); the conjunction on an edge model under a compromised channel is the unoccupied gap. **Robustness-under-compromise is the differentiating contribution that makes the coordination deployable** (with a measured detection-and-safety envelope); raw traffic-performance is a secondary, honestly-reported axis (floor + characterisation, Section 8). The SLM is **load-bearing on one job a heuristic cannot do**: disambiguating a detector-flagged anomaly (a genuine incident needing escalation vs a spoofed emergency engineered to grab preemption) using cross-junction context, where a fixed per-junction rule has no basis to decide. Its value on this job is a **characterisation, not a competition**: the SLM is measured against a well-tuned deterministic rule as a reference, not a bar it must clear, and a clean null (the rule suffices, the value of edge reasoning is bounded) is reported as a valid finding, same as a positive result. It is not claimed to beat a heuristic at routine phase selection (it does not, and it is slower); it is deployed only on flagged-anomaly states.
...
| Headline contribution | Performance; vs the crypto/integrity layer as-is; vs coordinated edge-SLM emergency response (robust under compromise) | **Coordinated edge-SLM emergency/incident response, robust under compromise** | The story is on-device SLM coordination clearing emergencies across junctions; robustness-under-compromise is the *differentiator* that makes it defensible. Signing/ledger/audit alone are commodity composition (the user's own framing: "slapped on"); the unoccupied gap is doing this on an edge model under an authenticated-but-lying neighbour. Coordination and emergency-handling are each precedented; the fusion under compromise is ours. |
...
**The claim that survives adversarial review:** to our knowledge no prior system uses a **frozen on-device SLM to coordinate multiple junctions for emergency and incident response**, and evaluates that coordination under a **signed-but-compromised channel**, characterising where consistency-based detection works and where it provably collapses. Coordination-for-emergencies is the headline; the compromised-channel robustness is the differentiating, defensible contribution (not the lead).
...
- **S3** anti-starvation: no approach skipped more than `[K: max_skip = 8]` consecutive decisions (a corroborated emergency may defer it by at most one min-green + yellow).
...
**Critical limitations.** Four bound the contribution honestly, beyond the individual claims/non-claims above. First, the simulator-to-reality gap: every number is relative-in-SUMO; sensor noise, actuation latency, and real driver behaviour are not modelled, so absolute figures do not transfer, only the relative comparisons within the simulator. Second, scalability beyond the corridor: the headline matrix is a synthetic corridor plus one real arterial (Lambeth); network-wide behaviour at city scale is untested and named explicitly as future work (Section 9, NICE), not implied by the corridor result. Third, the single-trust-domain assumption: one city authority, one admin key, and a permissioned registry are load-bearing throughout; a multi-authority or federated deployment (mutually distrusting operators, e.g. adjoining boroughs) is out of scope, and would need the ledger option this proposal demotes to a justified-or-not aside (Section 2). Fourth, what a null on the SLM characterisation would mean: if the frozen SLM shows no statistically and practically significant value over the tuned rule on the ambiguous set (Section 8), the contribution does not collapse, it re-centres on the trust-preserving coordination layer per the pre-committed decision rule (Sections 1 and 8), and the null is reported as a bound on where on-device reasoning helps, not a result to hide.
