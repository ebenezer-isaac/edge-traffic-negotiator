...
**qwen3-0.6b-ft0 / euston_peakhour / s19 (+21.8%) — mixed (seed-hard for all arms).** ft0's composition is exactly at its own norms (anti-starv 114 vs 114, div-served 84 vs 86, override 0.472 vs 0.465) — nothing anomalous in the decision stream. But ft1 (+18.3%) and phi4mini (+18.7%) also degrade here, and this is the *easiest* baseline seed of the topology (rank 1/34, 172.3 s = 0.72× median): the fixed-time plan got lucky on this realisation, inflating everyone's rel%. Chain ✓.
...
1. **It is not the seed's demand being objectively hard.** Catastrophic seeds are *baseline-easy or median* realisations — ranks 1, 4, 5, 7, 8, 10, 14, 16, 18, 20 of ~35; none in the top-baseline-delay tail. The fixed-time baseline never blows up on these seeds; only controllers do. (Low baselines do inflate rel% somewhat, but the absolute SLM delays are also 1.2–2.0× the arm's own topology median, so the collapses are real.)
