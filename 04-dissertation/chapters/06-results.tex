...
determine which delay numbers are comparable (\S\ref{sec:res-guards}) and
the demand calibration they forced, which yields the headline result
(\S\ref{sec:res-calib}); the authorship finding
(\S\ref{sec:res-authorship}); the failure forensics the audit layer makes
possible (\S\ref{sec:res-forensics}); the audit, trust,
emergency-preemption and authorisation-re-test results
(\S\ref{sec:res-audit}, \S\ref{sec:res-trust}, \S\ref{sec:res-ev},
...
Adding a naive fixed-time floor to every comparison was the most
consequential rigour decision of the project. In the exploratory sweep
(three seeds per cell, daily-resolution demand) MaxPressure ---
throughput-optimal in theory \cite{varaiya2013maxpressure} --- appeared
\emph{worse than naive fixed-time on every real London topology} (Euston
$+15\%$, Old Street $+5\%$, Bloomsbury $+3\%$) while dominating the
synthetic grids ($-27\%$ on grid4$\times$4, $-19\%$ on grid3$\times$3),
and an earlier draft made that a headline claim. It does not survive
replication: Table~\ref{tab:mpfloor} recomputes the comparison paired per
seed over the thirty disjoint evaluation seeds.
...
draw}. On Old Street --- the study's most complex geometry, five arms and
seventeen movements --- MaxPressure is \emph{significantly better} than
fixed-time, contradicting the exploratory result outright. On Bloomsbury
the two are indistinguishable. Only on the saturated Euston corridor does
MaxPressure remain worse in the mean, without reaching significance
($p{=}.21$) given the per-seed spread; the CI on the mean relative change
excludes zero while the paired test on absolute delay does not, a
divergence caused by skew and a reminder that a mean is a fragile summary
on this topology.

I therefore retract the general claim. What the evidence supports is
narrower: MaxPressure's advantage over a naive plan is \emph{unreliable}
on irregular real geometry under measured demand --- real on one topology,
absent on another, negative-but-uncertain on a third --- while uniform and
substantial on the regular synthetic grids the literature favours.
...
against 51--72\% for local SLMs, yet a near-chance local model still won
closed-loop cells. The controller's value is systemic --- SLM, shield and
gating together --- and this battery is open-loop by construction, stated
as such.

An explanatory two-way ANOVA (model $\times$ topology, two models over
four topologies, three seeds per cell) confirms the structure: topology
dominates (partial $\eta^2{=}0.97$), the model matters
($\eta^2{=}0.83$), and the two interact strongly (interaction
$F{=}32.2$, $p{<}0.001$, partial $\eta^2{=}0.86$) --- which SLM helps or
hurts is decided topology-by-topology, a further argument against pooled
claims. At three seeds per cell this is exploratory replication, not the
powered design.
...
significantly so on three of the eight arms (two-sided $p$ .017--.026,
with a fourth at $p{=}.053$), while remaining statistically
indistinguishable from MaxPressure. This is not a null; it is a measured harm, and it refutes
...
Paired $t$-tests on means hide structure: the per-seed distributions are
heavily skewed by the catastrophic tail of \S\ref{sec:res-forensics}.
Table~\ref{tab:robust} therefore adds 95\% confidence intervals and a
Wilcoxon signed-rank test, which asks whether the controller helps on a
...
controller \finding{significantly beats the floor on the typical seed}
($p{=}.011$) while its mean effect is not significant ($p{=}.202$); stock
shows the same pattern on Euston ($p{=}.018$ against $p{=}.117$). The
wide, zero-straddling intervals sit precisely on the topologies with the
heaviest failure tails, so the honest characterisation is not ``no
effect'' but \emph{typical-case improvement with a catastrophic tail}:
help on most seeds, bad failures on a few, and the few erase the mean.
That points at tail control --- a hard per-decision timeout or an outlier
detector reverting to the classical policy --- rather than at better
average decisions.
...
\finding{Only Old Street is measured in a free-flowing regime}: 85\% of
vehicles complete and teleports are effectively absent, so mean delay is
well-posed --- and it is also where the fine-tuned controller
significantly beats the floor on the typical seed
(Table~\ref{tab:robust}). The cleanest regime yields the clearest
result.

\risk{Uncalibrated Bloomsbury is a gridlock-regime measurement.} Under
15\% of vehicles complete and over a hundred are teleported out of
deadlock, so its ``mean network delay'' is computed over a network that
never clears, with the worst journeys removed by the simulator. The
original $+2.6\%$ stock harm is therefore regime-scoped. It survives the
guard test in direction --- the stock arm completes fewer vehicles than
baseline (231 vs 251) with more teleports (135.6 vs 118.5), so throughput
corroborates the harm --- but the comparison is between two
congested-failure modes, and \S\ref{sec:res-calib} supplies the remedy.

Euston sits between, at 44--50\% completion, and there the SLM arms
\emph{complete more vehicles than the baseline} (380 and 392 against 345)
with no significant delay advantage --- a throughput improvement a
delay-only report would have missed, and the reason both metrics are
mandated.
...
The reader is entitled to ask whether this is a principled correction or
a search for a more favourable test bed. In favour of the former: the
guard metrics that condemned the map were committed in
\S\ref{sec:met-metrics} beforehand; the diagnosis is outcome-independent,
since a longer horizon degraded clearance further; the 40\% target
matched the already-calibrated Old Street's completion rate rather than
any effect size; and Old Street had received the same treatment earlier.
...
What settles it is uniform application of the threshold: Euston peak-hour
also fails the completion guard (44--50\%), and it is the topology where
SLM control looks \emph{worst} (\S\ref{sec:res-factorial}), so
calibrating it risks the conclusion rather than protecting it.
\todo{report the Euston calibration outcome here whichever way it
falls.}
...
\risk{22.7\%} against the floor --- an order of magnitude beyond the
$+2.6\%$ measured on the same map oversaturated --- while both fine-tuned
students are statistically indistinguishable from the floor. The
head-to-head is the largest, most certain effect in this dissertation:
fine-tuning reduces delay by \finding{$18.3\%$} against the
identically-compiled stock model ($[-19.7, -16.9]$,
$p{=}8.5{\times}10^{-19}$), \finding{winning 30 of 30 seeds}; the Phi
student reproduces it exactly ($-18.3\%$, 30/30,
$p{=}4.5{\times}10^{-19}$). The mechanism: a deadlocked network is already
at its worst, with a ceiling on further degradation and teleports
truncating exactly the worst journeys, so good and bad control compress
toward the same mean; a flowing network gives a poor controller room to
do damage. Throughput corroborates the reading --- stock completes 76.4\%
of vehicles against the fine-tuned 87.4\% --- and the stock arm's decision
profile is unchanged (484 valid proposals of 546 decisions, no fallbacks,
normal latency), ruling out degraded serving.

\finding{This also bounds the scale-saturation result instead of leaving
it a non-rejection.} On the calibrated map the 3.8B and 0.6B students
differ by $+0.03\%$ with a 95\% confidence interval of $[-0.30, +0.36]$:
any difference between the scales is smaller than a third of a percent of
delay. I stop short of calling this a demonstrated equivalence, which
would require an equivalence margin pre-registered against an operational
threshold (a TOST procedure) that this study did not define; what the
interval supports is that six times the parameters buys, at most, a
practically negligible difference.

A methodological conclusion follows. \finding{Evaluating signal
controllers on oversaturated networks systematically understates effect
sizes}: the original cells alone would have concluded a marginal $1.8\%$
of doubtful significance where the clearing network yields $18.3\%$ on
every seed. The literature evaluates predominantly on synthetic grids
under demand rarely reported in clearance terms (\S\ref{sec:bg-rl}), and
its own calibration for the whole LLM-versus-MaxPressure gap is 1--2\%
\cite{dglight2026}, so compression of published effect sizes by
saturation deserves attention. Reporting completion and teleport counts
alongside delay costs nothing and would settle it.
...
earlier draft conflated them, and the distinction changes the
interpretation.
...
profile: on Euston peak-hour MaxPressure is $+19.8\%$ against the floor
in the mean (\S\ref{sec:res-maxpressure}) while the fine-tuned arms sit
at $-3.9\%$. And the remaining one-in-ten divergences are where the arms
differ from both baselines. Distinguishing imitation from independent
