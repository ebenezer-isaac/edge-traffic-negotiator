...
\end{axis}
\end{tikzpicture}
\caption{Closed-loop delay against the fixed-time floor, $n{=}30$ training-disjoint
seeds per cell. Negative is better than the floor. The distillation effect is the gap
between the two bars: it is largest on the calibrated Bloomsbury grid, where the
fine-tuned student beats the floor by $8.8\%$ while the stock model loses to it by
$11.9\%$, and it does not reproduce at that size on the calibrated corridor.}
\label{fig:headline}
\end{figure}
