...
$\pm0.36\%$ of closed-loop delay. Whether accuracy becomes delay depends on
whether the test network can express it: on saturated maps, over 30
training-disjoint seeds, no arm significantly beats a fixed-time baseline
--- but on the same grid demand-calibrated so that traffic actually flows,
the stock model degrades delay by 22.7\% while the distilled one matches
the baseline exactly, an \emph{18.3\% improvement on 30 of 30 seeds}
($p{\approx}10^{-19}$) that gridlock had been masking. Everywhere,
fine-tuning raises the SLM's share of all served actuations from
...
MaxPressure falls from 0.46--0.60 to 0.04--0.18. We argue authorship is
the property an audit needs --- an audit of the stock deployment could
attribute two decisions in five to the learned component, against up to
three in four after distillation, a deterministic timer authoring the
rest --- and we state the deflationary reading, that distillation partly
...
needed to see it.} On an oversaturated grid, distillation is worth under
two points; on the same grid calibrated to flow, 18.3\% on 30 of 30 seeds.
...
accuracy still decouples from delay --- yet fine-tuning raises the model's
share of served actuations by up to 17 points and cuts its divergence from
the classical reference four-fold. We define authored share on the
actuation denominator, the quantity a decision audit needs, and separate
