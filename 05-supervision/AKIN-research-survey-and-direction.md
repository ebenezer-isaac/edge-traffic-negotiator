...
**Title:** *The Edge Negotiator: trustworthy degradation of SLM-coordinated traffic signal control under a compromised coordination channel.*
...
A corridor of small language-model agents (Phi-4-mini, 3.8B, frozen, one per traffic junction) coordinate signal timing over a **signed** message channel, on top of a classical **MaxPressure** controller that stays in charge by default and acts as a safety shield. The research question is not "can the AI beat the classical controller on normal traffic" (it cannot, and we say so). It is: **when a neighbouring junction's messages are authenticated but compromised — a valid key in the wrong hands — how does the system degrade, and how does it defend itself?** The headline experiment spoofs a fake emergency to try to commandeer green time, and shows the system refuses it because preemption is gated on *physical corroboration*, not on the signature alone.
...
| Headline contribution | The crypto/integrity layer as-is; vs performance; vs robustness-under-compromise | **Robustness under compromised coordination** | Signing/ledger/audit alone are commodity composition. The unoccupied result is detecting and safely degrading under an *authenticated-but-lying* neighbour — the case signatures and blockchain cannot catch. |
