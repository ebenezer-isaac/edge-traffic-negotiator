> **SUPERSEDED HISTORICAL SURVEY NOTE (pre-2026-05-31 pivot).** Describes the retired Lambeth/blockchain/Qwen framing; the current thesis is in specs/001-edge-negotiator/MASTER-SPEC.md (Euston A501; self-referential coupling; CT-style accountability; Phi-4-mini).
...
| RESCO | RL Benchmarks for TSC | NeurIPS 2021 | https://datasets-benchmarks-proceedings.neurips.cc/paper/2021/file/f0935e4cd5920aa6c7c996a5ee53a70f-Paper-round1.pdf | — | IDQN, IPPO, MPLight, FMA2C, FixedTime, MaxPressure, MaxWave | SUMO Cologne/Ingolstadt/SLC | Reproduces MPLight ~11% gain over MP; flags non-robustness to sensing | SUMO-based RL benchmark — closer to a real-corridor SUMO setup than the synthetic CityFlow grids |
...
| MA2C-TSC | Multi-Agent DRL for Large-Scale TSC | IEEE TITS 2020 | https://arxiv.org/pdf/1903.04527 | — | IQL, IA2C, MaxPressure | SUMO 5×5 grid, Monaco | Decentralised MA2C beats IQL on 5×5 grid | The 5×5 SUMO grid is structurally comparable to the dissertation's synthetic 2×2 unit-test fixture (the real substrate is the Euston Road A501 corridor, not a synthetic grid) |
...
| Edge-Quant | Sustainable LLM Inference: Quantized Variants on Edge | 2025 | https://arxiv.org/pdf/2504.03360 | Gemma-2 2B, Llama-3.2 1B, Qwen-2.5 0.5/1.5B; 28 GGUF variants | Edge SBC | — | Q4_K_M typically best accuracy/energy/latency knee | Direct guidance for the frozen Phi-4-mini GGUF quantization choice |
...
| CollusionVeh | Attacking DRL TSC with Colluding Vehicles | 2021 | https://arxiv.org/pdf/2111.02845 | — | DRL TSC | SUMO | Coordinated falsified V2X telemetry breaks DRL; effect decreases with fleet size | Direct V2I-spoofing threat model — motivates the corroboration/quorum-anchored accountability log; colluding keys (≥2) are the out-of-scope hypothesis-failure axis in the current thesis, not a defeated attack |
...
| CoT-Faith | Reasoning Models Don't Always Say What They Think | 2025 | https://arxiv.org/pdf/2505.05410 | Claude-3.7-Sonnet, DeepSeek-R1 | hint-injection | — | **CoT faithfulness <20%** for several hint types; outcome RL plateaus | The strongest open-empirical paper motivating why the dissertation's SLM reasoning lives only in a firewalled, non-evidential internal note, validated against cited statute and never trusted as a verdict — not a standalone CoT-faithfulness experiment |
| BC-TSC | Blockchain-Based Architecture for TSC | 2019 | https://arxiv.org/pdf/1906.02628 | — | I-SIG controller | — | Mitigates I-SIG single-vehicle congestion attack | Cited prior art for non-repudiation ≠ truthfulness (Li-Nejad-Zhang); the accountability layer's mechanism is Certificate-Transparency-style (conceded to prior art), not a blockchain contribution |
...
**Note (2026-07-21):** the framing below predates the current thesis and originally argued for a joint-tuple novelty
claim (sub-7B SLM × edge hardware × a Lambeth/Southwark SUMO grid × a permissioned-blockchain audit ledger × a
CoT-faithfulness experiment). That claim is superseded by
[`../specs/001-edge-negotiator/MASTER-SPEC.md`](../specs/001-edge-negotiator/MASTER-SPEC.md). The frontiers this
literature actually establishes remain valid background; the novelty statement at the end has been rewritten to the
current thesis.

The literature shows three unambiguous frontiers, but **none of them intersect at the point Edge Negotiator occupies**.

**Frontier 1 — Sub-7B SLM as runtime traffic controller** is now established. Traffic-R1 (Qwen-2.5-3B), CuraLight (Gemma-3), HeraldLight (Llama-3.1-8B) and LLMLight's LightGPT-0.5B variant prove sub-7B/8B models can match Adv-CoLight on standard CityFlow benchmarks. **However, every one of these papers either (a) runs the model on a cloud-class GPU (T40, A100) or merely *claims* edge feasibility, or (b) when the model is actually placed on edge hardware (Edge-Orin-Bench, Edge-SLM-Energy, Edge-Hailo), the workload is generic MMLU/WikiText, not a closed-loop traffic-signal task.** No published paper reports tokens/sec, watts, P99 phase-decision latency, or thermal-throttling behaviour for Phi-4-mini executing a citation-faithful legal-reasoning note or a disambiguation decision on a Jetson Orin Nano, Hailo-10H, or Coral-class device inside a SUMO loop.

**Frontier 2 — Real-corridor realistic networks.** CityLight, HeraldLight, Traffic-R1 push to NY-196 and Beijing-13952, but always on CityFlow synthetic flows from Hangzhou/Jinan/NY. **No LLM-TSC paper uses a real UK corridor OSM extract in SUMO with calibrated TfL counts.** The closest SUMO grids are MA2C's synthetic 5×5 and RESCO's Cologne/Ingolstadt; Edge Negotiator's own synthetic 2×2 grid is a unit-test fixture only, with the real substrate being the Euston Road (A501) 3–4 signal stretch.

**Frontier 3 — Safety + auditability.** SafeLight, CFLight (action shielding), iLLM-TSC and LA-Light (LLM with classical fallback), SALT-V (V2I auth) and BC-Enforce (a cited permissioned-ledger audit precedent) each cover one slice. **No paper combines (i) a *deterministic* real-time gate that refuses signed-but-uncorroborated emergency preemption and clears physically-corroborated real emergencies, (ii) a Certificate-Transparency-style, quorum-anchored, cross-audited accountability log over every decision (mechanism conceded to prior art, not claimed novel), and (iii) a measured characterisation of the self-referential coupling between the attack lever (signal phase) and the honest-witness coverage gate that conceals it.**

The unoccupied intersection — and Edge Negotiator's novelty — is therefore not a performance tuple but the **self-referential coupling**: the preemption attack controls the signal phase, which is also the variable that gates honest-witness coverage, so executing the attack opens the very coverage desert that conceals it, stated as a conditional lemma and measured once, severely, on the real Euston Road (A501) corridor, alongside the canonical free-deviation boundary of what the deterministic gate + accountability log can and cannot hold accountable. A frozen, on-device **Phi-4-mini** SLM (the only model considered) is the mandatory, self-contained co-equal contribution, but is NOT the instrument of the boundary map. Each supporting element (sub-7B SLM control, real-corridor SUMO, a signed audit log) exists individually in prior work; the coupling + its measured boundary does not.
